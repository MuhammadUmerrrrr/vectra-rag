...
frontend (React)  <--REST-->  backend (FastAPI)
                                  ...
                                  └── OllamaClient     Groq + Hugging Face APIs
```
...
- **Documents tab**: real text is chunked (250 words, 30-word overlap)
  and embedded via Hugging Face's inference API, stored in a second HNSW
  index.
...
# Set environment variables GROQ_API_KEY and HF_API_KEY
...
- Built a full RAG pipeline (chunk → embed → retrieve → generate)
  using Groq + Hugging Face, with visual retrieval feedback
